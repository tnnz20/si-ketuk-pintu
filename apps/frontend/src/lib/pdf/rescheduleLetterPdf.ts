...
export async function generateRescheduleLetterPdf(
  ...
  pdf.text('Kepada Yth.', marginLeft, y);
  y += line;
  pdf.text(`Sdr. ${request.pimpinan_rombongan} (Pimpinan Rombongan)`, marginLeft, y);
  y += line;
  pdf.text(request.nama_instansi, marginLeft, y);
  y += line;
  pdf.text('di Tempat', marginLeft, y);
  y += 8;
  ...
  const segments = [
    ...
    {
      text: `dengan nomor registrasi #${request.token}, kami sampaikan bahwa jadwal kunjungan Saudara `,
      bold: false,
    },
  ...
  for (const segment of segments) {
    ...
    for (const word of words) {
      ...
      const token = `${word} `;
      if (x + pdf.getTextWidth(token) > pageWidth - marginRight) {
        x = marginLeft;
        y += line;
      }
      pdf.text(token, x, y);
      x += pdf.getTextWidth(token);
    }
  }
  y += 8;
  ...
  const rows = [
    ...
    [
      ...
      { content: request.tema_kunjungan, colSpan: 2 },
    ],
    [
      { content: 'Jumlah Tamu', colSpan: 1 },
      { content: `${request.jumlah_tamu} Orang`, colSpan: 2 },
    ],
  ];
  ...
  y += 5;
  const ending = pdf.splitTextToSize(
    'Demikian surat pemberitahuan ini disampaikan untuk dapat dipergunakan sebagaimana mestinya. Atas perhatian dan kerjasamanya, diucapkan terima kasih.',
    contentWidth,
  ) as string[];
  ...
  pdf.text('Admin / PIC Layanan SI KETUK PINTU', centerX, y, { align: 'center' });
  y += line;
  pdf.text('Sekretariat DPRD Kabupaten Tapin', centerX, y, { align: 'center' });
  y += 20;
