...
export function getArchives(
  ...
  return api(`/api/admin/archives?${params}`);
}
...
export async function uploadDocumentationImages(
  ...
  return api(`/api/admin/archives/${id}/documentations`, { method: 'POST', body: form });
}
...
export function deleteDocumentationImage(
  ...
  return api(`/api/admin/archives/${id}/documentations/${attachmentId}`, { method: 'DELETE' });
}

export function uploadDaftarAbsen(id: string, file: File): Promise<{ attachment: Attachment }> {
  const form = new FormData();
  form.append('file', file);
  return api(`/api/admin/archives/${id}/daftar-absen`, { method: 'POST', body: form });
}

export function deleteDaftarAbsen(id: string): Promise<{ message: string }> {
  return api(`/api/admin/archives/${id}/daftar-absen`, { method: 'DELETE' });
}
...
export function downloadArchiveAttachment(
  ...
  return api(`/api/admin/archives/${id}/attachments/${type}/${attachmentId}`);
}
